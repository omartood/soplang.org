...
`abn` waxa uu metelaa tirooyin dhan (…,-2,-1,0,1,2,…). Waxaad qeexi kartaa si _dynamic_ (`door`) ama si _static_ (`abn`).

<SoplangCodeWindow title="integers.sop" code={`// Literals & declaration
abn da = 21
...
Qaybintu (`/`) waxay bixin kartaa natiijo _jajab_ haddii aysan si buuxda u qaybsamin—haddii aad rabto integer oo keliya, isticmaal `abn()` si aad u _truncation_ sameyso.

<SoplangCodeWindow title="integer_truncation.sop" code={`abn natiijo_dhan = abn(10 / 3)  // 3 (truncation)`}/>

## Decimals — `jajab`

`jajab` waxa uu metelaa tirooyinka leh meel-dhaaf (floating/decimal). Waxaa ku habboon xisaab sax ah oo leh jajab.

<SoplangCodeWindow title="decimals.sop" code={`// Literals & declaration
jajab pi = 3.14159
...
<SoplangCodeWindow title="check_decimal_type.sop" code={`door n = 2.75
qor(nooc(n))  // "jajab"`}/>
...
<SoplangCodeWindow title="mixing_abn_jajab.sop" code={`abn a = 5
jajab b = 2.0
...
<SoplangCodeWindow title="numbers_quick_check.sop" code={`// From input → number
